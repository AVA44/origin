...
<form method="POST" action="{{ url('/inventory', $inventory->id) }}">
    ...
    <div class="form-contents">
        ...
        <input id="form-content1" type="text" name="name" value="{{ old('name') == '' ? $inventory->name : old('name') }}">
    </div>
    <div class="form-contents">
        <label for="form-content2">賞味期限</label>
        <input id="form-content2" type="date" name="date" value="{{ old('date') == '' ? $inventory->date : old('date') }}">
    </div>
    <div class="form-contents">
    ...
    <div class="form-contents">
        ...
        <input id="form-content4" type="text" name="stock" value="{{ old('stock') == '' ? $inventory->stock : old('stock') }}">
    </div>
    <div class="form-contents">
        <label for="form-content5">納入時個数</label>
        <input id="form-content5" type="text" name="purchase" value="{{ old('purchase') == '' ? $inventory->purchase : old('purchase') }}">
    </div>
    <div class="form-contents">
        <label for="form-content6">単価</label>
        <input id="form-content6" type="text" name="unit_price" value="{{ old('unit_price') == '' ? $inventory->unit_price : old('unit_price') }}">
    </div>
    <div class="form-contents">
...
<form class="destroy" method="post" action="{{ url('/inventory', $inventory->id) }}">
    <input type="hidden" name="delete-flag" value=1>
